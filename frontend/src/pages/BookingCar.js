...
//       setCar(cars.find((o) => o._id === match.params.carid));
//     }
//   }, [cars, match.params.carid, dispatch]);

//   // recalc total amount whenever hours or driver changes
//   useEffect(() => {
...
//     const reqObj = {
//       token: { email: "test@example.com", id: "fake-txn" }, // fake payment
//       user: JSON.parse(localStorage.getItem("user"))._id,
//       car: car._id,
//       totalHours,
//       totalAmount,
...
//         {/* modal for booked slots */}
//         {car.name && (
//           <Modal
//             open={showModal} // ✅ AntD v5 uses open instead of visible
//             closable={false}
//             footer={false}
//             title="Booked time slots"
//           >
...
// frontend/src/pages/BookingCar.js
import { Col, Row, Divider, DatePicker, Checkbox, Modal } from "antd";
import React, { useState, useEffect } from "react";
import { useSelector, useDispatch } from "react-redux";
import DefaultLayout from "../components/DefaultLayout";
...
function BookingCar({ match }) {
  ...
  function handleBooking() {
    const reqObj = {
      ...
      token: { id: "fake-payment-token" }, // fake token
    };
        ...
        <Col lg={10} sm={24} xs={24} className="text-right">
          ...
          {from && to && (
            <div>
              ...
              <h3>Total Amount: {totalAmount}</h3>
              <button className="btn1" onClick={handleBooking}>Book Now</button>
            </div>
          )}
        </Col>
